...
int main(int argc, char *argv[])
{
  ...
  // assume purely vector couplings and mZ' >> mQv
  // Width = [ mZ' / 12 pi ] * [ sum_qSM(3 gSM^2) + sum_qHV(nGauge gHV^2) ]
  double Zwidth;
  ...
  Zwidth = pythia.particleData.mWidth(4900023);
  std::cout << "Z' width set to: " << Zwidth << std::endl;

  // other variables for hidden valley showering
  double Lambda = pythia.settings.parm("HiddenValley:Lambda");
  std::cout << "Hidden Valley Lambda: " << Lambda << std::endl;
  double mq = pythia.particleData.m0(4900101);
  std::cout << "Hidden Valley quark mass: " << mq << std::endl;
  ...
  lundOut << "#evt"
          << std::setw(4) << "jet"
          << std::setw(lundWidth-3) << "darkEfrac"
          << std::setw(lundWidth-4) << "clustE"
          << std::setw(lundWidth-3) << "darkPtfrac"
  ...
  for (int iEvent = 0; iEvent < nEvents; ++iEvent) {
    ...
    // Number of "final" still hidden gv, gammav and hadronv in current event.
    int nGluonvNow  = 0;
    int nGammavNow  = 0;
    int nHadronvNow = 0;
    ...
    for (int iJet = 0; iJet < int(sortedJets.size()); ++iJet) {
      ...
      while (j.has_parents(j1, j2))
      {
        ...
        lundOut << iEvent
                ...
                << std::setw(lundWidth-3) << darkHadronEnergyFrac
                << std::setw(lundWidth-3) << clusterE
                << std::setw(lundWidth-3) << darkHadronPtFrac
